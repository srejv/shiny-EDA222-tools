...
joint.shapes.basic.PortsModelInterface = {
    ...
    initialize: function() {
        ...
        this.on('change:inPorts change:outPorts change:variablePorts', this.updatePortsAttrs, this);
    ...
    updatePortsAttrs: function(eventName) {
        ...
        _.each(this.get('variablePorts'), function(portName, index, ports) {
            var portAttributes = this.getPortAttrs(portName, index, ports.length, '.variablePorts', 'variable');
            this._portSelectors = this._portSelectors.concat(_.keys(portAttributes));
            _.extend(attrs, portAttributes);
        }, this);
    ...
    getPortSelector: function(name) {
        ...
        if (index < 0) {
            ...
            if (index < 0) {
            	slector = '.variablePorts';
            	index = this.get('variablePorts').indexOf(name);
...
joint.shapes.basic.PortsViewInterface = {
    ...
    renderPorts: function() {
        ...
        var $variablePorts = this.$('.variablePorts').empty();
        ...
        _.each(_.filter(this.model.ports, function(p) { return p.type === 'variable' }), function(port, index) {
            $variablePorts.append(V(portTemplate({ id: index, port: port })).node);
        });
    }
};
